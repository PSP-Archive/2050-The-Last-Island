...
void EmitWaterPolys (msurface_t *fa)
{
	...
	for (const glpoly_t* p = fa->polys; p; p = p->next)
	{
		...
		while (src != last_vertex)
		{
			...
			dst->st[0] = gl_waterripple.value * (os + turbsin[(int) ((ot * 0.125f + real_time) * turbscale) & 255]) * scale;
			dst->st[1] = gl_waterripple.value * (ot + turbsin[(int) ((os * 0 + real_time) * turbscale) & 0]) * scale;
